...
export async function updateBookings() {
  ...
  try {
    ...
    if (bookings.rows.length > 0) {
      console.log(
        `[${jobName}] Found ${bookings.rows.length} bookings. Starting transaction...`
      );
      await db.transaction(async (trx) => {
        console.log(`[${jobName}] Transaction started.`);
        for (const booking of bookings.rows) {
          console.log(`[${jobName}] processing this booking`, booking);
          const { id, meeting_status, joined_by, payment, status, starts_on } =
            booking;
          console.log(
        ...
        console.log(`[${jobName}] Transaction block complete.`);
      });

      logMessage(jobName, "All eligible bookings updated successfully on.");
      console.log(`[${jobName}] All eligible bookings updated successfully.`);
    } else {
